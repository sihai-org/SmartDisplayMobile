  ...
  static Future<void> handle(
    ...
    try {
      ...
      if (AuditMode.enabled) {
        final curDisplayId = ref
            .read(bleConnectionProvider)
            .bleDeviceData
            ?.displayDeviceId;
        if (curDisplayId != null &&
            curDisplayId == AuditMode.mockDisplayDeviceId) {
          context.go(
            '${AppRoutes.home}?displayDeviceId=${AuditMode.mockDisplayDeviceId}',
          );
          return;
        }
      }
      ...
      // 2. 前往连接
      AppLog.instance.info(
        '[DeviceEntryCoordinator.handle] go connecting',
        tag: 'Binding',
      );
      ref.read(appStateProvider.notifier).setScannedData(qrData);
      if (context.mounted) {
        context.go(
          '${AppRoutes.deviceConnection}?displayDeviceId=${qrData.displayDeviceId}',
        );
      }
    } catch (e) {
      ...
      AppLog.instance.warning(
        '[DeviceEntryCoordinator.handle] parse failed -> show raw content page',
        tag: 'Binding',
        error: e,
      );
